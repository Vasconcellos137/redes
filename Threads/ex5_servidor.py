...
def iniciar_servidor():
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server:
        server.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1) #P tentar diminuir o problema de ter.. 
        server.bind((HOST, PORT))                                    #..q alterar a porta p usar          
        server.listen()
        ...
        while True:
            ...
            thread = threading.Thread(
                target=atender_cliente,  #É uma thread destinada/iniciada pelo server p atender o cliente
                args=(conn, addr),
                daemon=True
            )
            thread.start()
